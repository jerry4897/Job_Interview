...
비메모리는 연산이나 논리와 같이 정보처리를 담당.(시스템 반도체).. ex, PC용 CPU, 스마트폰 AP(응용프로세서)
각종 IT장치에 센서를 통해 들어오는 정보들을 분석하고 필요한 작업을 수행.

D(ynamic)RAM  백만분의 몇초마다 reflah시켜줘야 저장된 내용이 유지.

S(tatic)RAM reflash 필요X. DRAM보다 반응속도 빨라 L1, L2캐쉬처럼 CPU와 긴밀하고 빠르게 데이터를 주고 받을 수 있도록 사용된다.
